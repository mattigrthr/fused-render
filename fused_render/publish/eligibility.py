...
def pyodide_imports() -> dict[str, str]:
    """``{top-level import name: Pyodide package name}`` for the pinned distribution.

    NOT a hand-kept guess: a checked-in projection of the distribution's own
    ``pyodide-lock.json`` (``pyodide_dist.import_map``, regenerated by
    ``scripts/refresh_pyodide_imports.py``). So "can a browser Python runtime
    import this?" is answered here by the same file that decides, at publish
    time, which wheel gets vendored — the scan and the build cannot disagree.

    Read through a function rather than a module constant so the JSON is loaded
    on the first scan, not on every import of the publish package.
    """
    from fused_render.publish.pyodide_dist import import_map
...
def scan(html_path: str, *, include: list[str] | None = None, exclude: list[str] | None = None) -> Eligibility:
    ...
    for key in py_files:
        ...
        for mod in imports:
            if mod in sys.stdlib_module_names or mod in local_names:
                ...
                continue
            if mod in available:
                packages.setdefault(available[mod], None)
                if mod in _NETWORK_IMPORTS:
                    network_seen = True
                continue
    ...
    if py_files and runtime is Capability.RUNTIME_PYODIDE:
        notes.append(
            "the app's Python runs in each reader's browser, so its SOURCE is part of the "
            "published page — "
            + ", ".join(sorted(py_files))
            + ". Anyone who opens the app can read it. Nothing in those files should be a "
            "secret: no API key, no private endpoint, no comment you would not put on the "
            "page itself."
        )
